<?php 
						...
						if($_SESSION['cod_recibo']==''){
							...
							$sql .= "where f.cod_forn = r.cod_forn and p.cod_cal = e.cod_cal ";
							$sql .= "and f.cod_forn = e.cod_forn and r.cod_recibo = e.cod_recibo ";
							$sql .= "and r.cod_recibo = '".$_SESSION['cod_recibo']."' order by e.cod_ent;";
						}
						...
						$html1 .= "
						<h2><div class='titulorel'>Relatório de Entrada de Calçados na Loja</div></h2>
						";
						...
						<td width='20%'><center><strong>QUANTIDADE DA ENTRADA</strong></center></td>
						<td width='20%'><center><strong>VALOR DA ENTRADA</strong></center></td>
						...
						while($info = mysql_fetch_array($data)){
							if($cont % 2 == 0){
								$html1 .= "<tr style='background:#eee;'><td>".$info['cod_ent']."</td>
								<td>".$info['nome_cal']."</td>
                                <td><center>".$info['numero']."</center></td>
								<td><center>".$info['qtde_entrada']."</center></td>
								<td><center> R$ ".number_format($info['preco_entrada'], 2,',','.')."</center></td>
								<td><center>".$info['qtde_estoque']."</center></td></tr>";
							}else{
								$html1 .= "<tr><td>".$info['cod_ent']."</td>
								<td>".$info['nome_cal']."</td>
                                <td><center>".$info['numero']."</center></td>
								<td><center>".$info['qtde_entrada']."</center></td>
								<td><center> R$ ".number_format($info['preco_entrada'], 2,',','.')."</center></td>
